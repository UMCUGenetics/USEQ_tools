    ...
    for io_map in step.details.input_output_maps:
        ...
        if current_step in Config.WORKFLOW_STEPS['SEQUENCING']['steps']['ISOLATION']['names']:
            ...
            if 'Library prep kit' in first_sample.udf:
                ...
                next_stages.append(Config.WORKFLOW_STEPS['SEQUENCING']['steps']['LIBPREP']['stage_nrs'][ first_sample.udf['Library prep kit'] ])
            else:
                if first_sample.udf['Platform'] in ['Oxford Nanopore','MinIon','PromethIon']:
                    if first_sample.udf['Sample Type'] == 'RNA total isolated':
                        ...
                        next_stages.append(Config.WORKFLOW_STEPS['SEQUENCING']['steps']['LIBPREP']['stage_nrs'][ 'USEQ - LIBPREP-ONT-RNA' ])
                    else:
                        # next_step = STEP_URIS['USEQ - LIBPREP-ONT-DNA']
                        next_stages.append(Config.WORKFLOW_STEPS['SEQUENCING']['steps']['LIBPREP']['stage_nrs'][ 'USEQ - LIBPREP-ONT-DNA' ])
                elif first_sample.udf['Platform'] == 'SNP Fingerprinting': #old fingerprinting workflow
                    # next_step = STEP_URIS[ 'USEQ - Fingerprinting' ]
                    next_stages.append(Config.WORKFLOW_STEPS['FINGERPRINTING']['steps']['FINGERPRINTING']['stage_nrs'][ 'USEQ - Fingerprinting' ])
                elif first_sample.udf['Platform'] == '60 SNP NimaGen panel':
                    next_stages.append(Config.WORKFLOW_STEPS['SEQUENCING']['steps']['LIBPREP']['stage_nrs'][ 'USEQ - LIBPREP-FINGERPRINTING' ])
            ...
            # else:
            next_stages.append(Config.WORKFLOW_STEPS['SEQUENCING']['steps']['POOLING']['stage_nrs'][ 'USEQ - Library Pooling' ])
            ...
            if sample_type == 'DNA library' or sample_type == 'RNA library': #Go to pool QC
                ...
                next_stages.append(Config.WORKFLOW_STEPS['SEQUENCING']['steps']['POOL QC']['stage_nrs'][ 'USEQ - Pool QC' ])
                ...
                if platform in ['Oxford Nanopore','MinIon','PromethIon']:
                    next_stages.append(Config.WORKFLOW_STEPS['SEQUENCING']['steps']['NANOPORE SEQUENCING']['stage_nrs']['Oxford Nanopore'])
                    ...
                    next_stages.append(Config.WORKFLOW_STEPS['SEQUENCING']['steps']['ILLUMINA SEQUENCING']['stage_nrs'][sequencing_platform])
                    ...
                    # else:
                    next_stages.append(Config.WORKFLOW_STEPS['SEQUENCING']['steps']['ILLUMINA SEQUENCING']['stage_nrs'][ platform ])
            ...
            next_stages.append(Config.WORKFLOW_STEPS['SEQUENCING']['steps']['POST SEQUENCING']['stage_nrs']['USEQ - Post Sequencing'])
            ...
            if current_step in ['USEQ - BCL to FastQ','USEQ - Process Raw Data']:
                ...
                if platform in ['60 SNP NimaGen panel','Chromium X'] or sample_analyses:
                    next_stages.append(Config.WORKFLOW_STEPS['SEQUENCING']['steps']['POST SEQUENCING']['stage_nrs']['USEQ - Analysis'])
                    run_finished(lims,Config.MAIL_SENDER, Config.TRELLO_ANALYSIS_BOARD, artifact )
                next_stages.append(Config.WORKFLOW_STEPS['SEQUENCING']['steps']['POST SEQUENCING']['stage_nrs']['USEQ - Ready for billing'])
