...
def updateProjectsFromLIMS( ):
	...
	for seq_process in SEQUENCING_PROCESSES:
	    pr_XML = api.getResourceByURI( BASE_URI + 'processes/?projectname=' + project['name'] + '&type='+ urllib.quote( SEQUENCING_PROCESSES[seq_process] ) )
	...
	for sequencer_dir in RAW_DATA_DIRS[platform]:
		...
		if os.path.isdir(os.path.join(sequencer_dir, item)):
			...
			if ( len(p_nodes) > 0 ):
			    ...
			    #PROJECT_INFO[flow_cell]['name'] = p_name
